...
### 2. Rank With Scanner
```
echo "TICKER1,TICKER2,..." | python3 ./scripts/scanner.py
```
...
```
cd robinhood-agentic && node rh-client.mjs call get_portfolio '{"account_number":"<ACCT>"}'
```
...
### 1. Check Positions
```
cd robinhood-agentic && node rh-client.mjs call get_equity_positions '{"account_number":"<ACCT>"}'
```

### 2. Get Current Quotes
```
cd robinhood-agentic && node rh-client.mjs call get_equity_quotes '{"symbols":["TICKER1","TICKER2"]}'
```
